"""
    Python scrypt to use the grlc-bakery's API
...
def send_request_bakery(function, parameters={}, type='api', get=1):
    """ Sends request to bakery's API.
        inputs:
            function: name (ex. getdifficulty)
            parameters: parameters to send in the GET request (dict)
            type: the sub-directory of the function
            get: for parameters that are sent with GET request, ex. url?parameter1=value1&parameter2=value2
                    or not, ex. url/value1/value2
        output:
            response content
    """
...
def extract_transactions(block_hash):
    ...
    if 'tx' in [b[0] for b in block.items()]:
        transactions = [t for t in block['tx']]
    else:
        transactions = []
